...
function main(){
		...
		render: function() {
			...
			for (var i = this.list.length - 1; i >= 0; i--) {
				if (_effect = this.list[i]) {
					...
					drawImg(gImg[_effect.imgId], _effect.x, _effect.y, 1, 1, 0);
	    		...
	    		for(var b = world.m_bodyList; b != null; b = b.m_next){  
					if (b.GetUserData()) {
			    		...
			    		if(_type == "fruit") {
			            	...
			            	if (_name != 'bear') {
								effect.addEffect(pos.x * worldScale, pos.y * worldScale, _name + 'Bg');
			            	};
							playBreakAnimate(b);
			            	world.DestroyBody(b);
			            }else if (_type == "blade") {
	...
	function handleMouseMove(e) {
		...
		if (gStatus == 'run') {
			...
			if (_startDate && Date.now() - _startDate >= 100) {
				...
				var _blade = createBox(_side >> 1, 0.1, (_startX + _endX )>> 1, (_startY + _endY) >> 1, _angle, b2Body.b2_kinematicBody, _userData);
	...
	function setMousePosition(e){
		mouseX = (e.clientX - canvasPosition.x) / worldScale;
